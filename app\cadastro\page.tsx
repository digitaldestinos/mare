...
export default function SignUpPage() {
  ...
  async function onSubmit(input: SignUpInput) {
    ...
    if (error) { setError("root", { message: "Não foi possível criar sua conta. Tente novamente." }); return; }
    if (data.session) { router.replace("/dashboard"); router.refresh(); } else { router.replace("/cadastro/confirmacao"); }
  }
  return <AuthShell eyebrow="Comece com calma" title="Crie sua conta" description="Um espaço simples para organizar sua vida financeira.">
    <form className="space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div><label className="mb-2 block text-sm font-medium" htmlFor="password">Senha</label><Input id="password" type="password" autoComplete="new-password" placeholder="Mínimo de 8 caracteres" {...register("password")} /><FieldError message={errors.password?.message} /></div>
      ...
      <Button className="w-full" type="submit" disabled={isSubmitting}>{isSubmitting ? "Criando conta…" : "Criar conta"}</Button>
      <p className="text-center text-sm text-muted-foreground">Já tem uma conta? <Link className="font-medium text-foreground hover:underline" href="/login">Entrar</Link></p>
    </form>
  </AuthShell>;
}
