...
export default function ResetPasswordPage() {
  ...
  async function onSubmit(input: ResetPasswordInput) {
    ...
    if (error) { setError("root", { message: "NÃ£o foi possÃ­vel atualizar sua senha. Solicite um novo link." }); return; }
    await createClient().auth.signOut();
    router.replace("/login");
  }
  return <AuthShell eyebrow="Acesso seguro" title="Crie uma nova senha" description="Escolha uma senha forte para continuar usando o MarÃ©."><form className="space-y-5" onSubmit={handleSubmit(onSubmit)} noValidate><div><label className="mb-2 block text-sm font-medium" htmlFor="password">Nova senha</label><Input id="password" type="password" autoComplete="new-password" placeholder="MÃ­nimo de 8 caracteres" {...register("password")} /><FieldError message={errors.password?.message} /></div><div><label className="mb-2 block text-sm font-medium" htmlFor="confirmPassword">Confirmar senha</label><Input id="confirmPassword" type="password" autoComplete="new-password" placeholder="Repita sua senha" {...register("confirmPassword")} /><FieldError message={errors.confirmPassword?.message} /></div>{errors.root && <p className="rounded-md bg-destructive/10 px-3 py-2 text-sm text-destructive">{errors.root.message}</p>}<Button className="w-full" type="submit" disabled={isSubmitting}>{isSubmitting ? "Salvandoâ€¦" : "Salvar nova senha"}</Button></form></AuthShell>;
}
