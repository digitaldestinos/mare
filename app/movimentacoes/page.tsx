...
import { useRecordQuickCapture } from "@/hooks/use-transactions";
...
export default function TransactionsPage() {
  ...
  const { data: wallets, isLoading: walletsLoading, isError: walletsError } = useWallets(user?.id);
  const wallet = wallets?.find((item) => item.active) ?? wallets?.[0];
  const { data: transactions, isLoading: transactionsLoading, isError: transactionsError, refetch } = useTransactions(user?.id, wallet?.id);
  const { data: categories } = useCategories(user?.id);
  const captureMutation = useRecordQuickCapture(user?.id, wallet?.id);
  ...
  useEffect(() => {
    if (!isLoading && user) {
      if (!walletsLoading && wallets && wallets.length === 0) { router.replace("/onboarding"); return; }
  ...
  }, [isLoading, router, user, wallet, wallets, walletsLoading]);
