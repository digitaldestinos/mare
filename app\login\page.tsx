...
export default function LoginPage() {
  ...
  async function onSubmit(input: LoginInput) {
    ...
    if (error) { setError("root", { message: "NÃ£o foi possÃ­vel entrar. Confira seu e-mail e senha." }); return; }
    router.replace("/dashboard");
    router.refresh();
  }
  return <AuthShell eyebrow="Bem-vindo de volta" title="Entre no MarÃ©" description="Acompanhe suas finanÃ§as com mais clareza e tranquilidade.">
    <form className="space-y-5" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <Button className="w-full" type="submit" disabled={isSubmitting}>{isSubmitting ? "Entrandoâ€¦" : "Entrar"}</Button>
      <p className="text-center text-sm text-muted-foreground">Ainda nÃ£o tem uma conta? <Link className="font-medium text-foreground hover:underline" href="/cadastro">Criar conta</Link></p>
    </form>
  </AuthShell>;
}
