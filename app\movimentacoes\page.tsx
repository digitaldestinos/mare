...
export default function TransactionsPage() {
  ...
  function interpret(value = source) {
    ...
    if (!result) { setPreview(null); setError("Digite uma descriÃ§Ã£o e um valor, como â€œUber 18â€."); return; }
    setPreview(result);
  ...
  function confirmCapture() {
    ...
    if (!wallet) { setError("Crie uma carteira antes de registrar sua primeira movimentaÃ§Ã£o."); setDialogOpen(false); return; }
...
function CapturePreview({ preview, walletName, onConfirm, onEdit }: Readonly<{ preview: QuickCaptureResult; walletName: string; onConfirm: () => void; onEdit: () => void }>) {
  return <Card className="mx-auto mt-8 max-w-lg animate-slide-up"><CardHeader className="flex-row items-center justify-between space-y-0"><div><p className="text-xs uppercase tracking-wide text-muted-foreground">PrÃ©via</p><h2 className="mt-1 text-base font-medium">Tudo certo para registrar?</h2></div><Badge className={preview.type === "income" ? "border-accent bg-accent/50 text-accent-foreground" : "bg-secondary"}>{preview.type === "income" ? "Receita" : "Despesa"}</Badge></CardHeader><CardContent><PreviewDetails preview={preview} walletName={walletName} /><div className="mt-6 flex gap-3"><Button className="flex-1" onClick={onConfirm}><Check size={16} /> Confirmar</Button><Button variant="outline" onClick={onEdit}><Pencil size={15} /> Editar</Button></div></CardContent></Card>;
}

function PreviewDetails({ preview, walletName }: Readonly<{ preview: QuickCaptureResult; walletName: string }>) {
  const rows = [["Valor", formatCurrency(preview.amount)], ["Categoria", preview.category], ["Carteira", walletName], ["Data", "Hoje"], ["DescriÃ§Ã£o", preview.description]];
